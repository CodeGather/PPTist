    ...
    const creatElement = (item: OrderDataList, type: string, child?: string, more?: string) => {
      ...
      switch (type) {
        ...
        case 'video':
          ...
          break
        default:
          drawText('点位名称', 'pointName', 0, 0)
          drawText(`<p style="text-align: center;">${more || child || item.name, more || child || item.name}</p>`, '', 0, 100)
          drawText(`<p style="text-align: center;">施工指引</p>`, '', 0, 280)
          drawShape(more || child || item.name, {left: 100})
          drawShape(more || child || item.name, {left: 280})
          break
      }
    }

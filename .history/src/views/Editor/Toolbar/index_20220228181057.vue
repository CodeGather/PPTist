<template>
  <div class="toolbar">
    ...
    <div class="content">
      {{JSON.stringify(currentPanelComponent[toolbarState])}}
      {{toolbarState}}
      <component 
        v-for="(item, index) in currentPanelComponent"
        :is="currentPanelComponent[toolbarState]" 
        :key="index"
        v-show="toolbarState.value = item.name"
        ></component>
    </div>
  </div>
</template>

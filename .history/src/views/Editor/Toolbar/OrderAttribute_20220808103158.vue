    ...
    <Dropdown 
      ...
      <template #overlay>
        <Menu>
          ...
          <MenuItem v-if="item.attrCode == 'material'" @click="creatElement(item, 'image')"><IconText /> 暂位图</MenuItem>
        </Menu>
      </template>
    </Dropdown>
  </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const { createTextElement, createTableElement, createImageElement } = useCreateElement()
    ...
    const creatElement = (item: OrderAttributeValueList, type: string, more?: string) => {
      switch (type) {
        ...
        case 'image':
          createImageElement(
            'https://dali-pord.oss-cn-shanghai.aliyuncs.com/static/material.png',
            'material',
          )
          break
        default:

<template>
  <div class="multi-position-panel" v-if="orderPageData">
    <Dropdown 
      ...
      <template #overlay>
        <Menu>
          <MenuItem v-for="(t, i) in item.photosText" :key="i" @click="creatElement(item, 'text', item.name)"><IconText /> {{t.name}}</MenuItem>
          <MenuItem v-for="(t, i) in item.videosText" :key="i" @click="creatElement(item, 'text', item.name)"><IconText /> {{t.name}}</MenuItem>

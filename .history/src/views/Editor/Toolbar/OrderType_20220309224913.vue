          ...
          <template v-if="item.serviceIds">
            <SubMenu v-for="(child, i) in item.serviceList" :key="'service_' + i">
              ...
              <SubMenu key="photosText" v-if="child.photosText.length > 0">
                ...
                <MenuItem v-for="(t, i) in child.photosText" :key="'photosText_' + i" @click="creatElement(item, 'image', t.name)"><IconPicture /> {{t.name}}</MenuItem>
                <SubMenu v-for="(t, i) in child.photosText" :key="'photosText_' + i" :title="t.name">
                  <MenuItem @click="creatElement(item, 'text', child.name, t.name)"><IconText /> 名称</MenuItem>
              ...
              <SubMenu key="videosText" v-if="child.videosText.length > 0">
                ...
                <SubMenu v-for="(t, i) in child.videosText" :key="'videosText_' + i" :title="t.name">
                  <MenuItem @click="creatElement(item, 'text', child.name, t.name)"><IconText /> 名称</MenuItem>
              ...
              <SubMenu key="optionsText" v-if="child.optionsText.length > 0">
                ...
                <SubMenu v-for="(t, i) in child.optionsText" :key="'optionsText_' + i" :title="t.name">
                  <MenuItem @click="creatElement(item, 'text', child.name, t.name)"><IconText /> 名称</MenuItem>

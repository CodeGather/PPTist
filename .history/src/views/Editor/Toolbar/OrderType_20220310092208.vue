...
import { CommonElementPosition } from '@/hooks/useCreateElement'
...
export default defineComponent({
  ...
  setup() {
    ...
    const creatElement = (item: OrderDataList, type: string, child?: string, more?: string) => {
      ...
      switch (type) {
        ...
        case 'video':
          ...
          break
        default:
          drawText(`<p style="text-align: center;">施工指引</p>`)
          drawShape(more || child || item.name)
    ...
    // 绘制图片范围
    const drawShape = (keyName: string, position?: CommonElementPosition) => {
      createImageElement(

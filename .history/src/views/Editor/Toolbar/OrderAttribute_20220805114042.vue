...
import { Slide, PPTTableElement, PPTElement, TableCell } from '@/types/slides'
      ...
      switch (type) {
        case 'text':
          if (item.attrCode === 'material') {
            createTable()
            const tableList:PPTElement[] = currentSlide.value.elements.filter(el => {
              return 'table' === el.type
            })
            const newTable = tableList.map((el:PPTElement) => {
              let tableItem:PPTTableElement = el as PPTTableElement
              tableItem.data[0] = tableItem.data[0].map((item:TableCell) => {
                item.text = '9999'
                return item
              })
              return el
            ...
            store.commit(MutationTypes.UPDATE_FILEDATA, { fileName: 'material' })
            // store.commit(MutationTypes.UPDATE_SLIDE, { fileName: fileDataConfig.value.fileName + item.attrName })

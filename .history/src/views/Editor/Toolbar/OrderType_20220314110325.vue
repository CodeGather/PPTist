<template>
  <div class="order-type" v-if="orderPageData">
    <Button 
      ...
      @click="creatElement({name: '服务属性名'}, 'parent', '服务属性名')"
      >服务属性名</Button>
    <Dropdown 
      :trigger="['click']" 
      ...
      <template #overlay>
         <Menu size="small">
          ...
          <MenuItem @click="creatElement(item, 'image', item.name, item.name)"><IconPicture /> 图片</MenuItem>
          <MenuItem @click="creatElement(item, 'video', item.name, item.name)"><IconVideoTwo /> 视频</MenuItem>
          <template v-if="item.serviceIds && item.isSub">
            <SubMenu v-for="(child, i) in item.children" :key="'service_' + i">
...
export default defineComponent({
  ...
  setup() {
    ...
    const creatElement = (item: OrderDataList, type: string, child?: string, more?: string) => {
      // 需要处理点位的标识
      store.commit(MutationTypes.UPDATE_SLIDE, { keyName: `服务属性……${item.name}${child ? '……' + child : ''}` })
      switch (type) {
        case 'image':

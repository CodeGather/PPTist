    ...
    <Dropdown 
      ...
      <template #overlay>
        <Menu>
          ...
          <MenuItem v-for="(t, i) in item.photosText" :key="i" @click="creatElement(item, 'image', item.name + t.name)">
            <RadioGroup 
              class="row" 
              button-style="solid" 
            >
              <RadioButton value="left" style="flex: 1;">名称</RadioButton>
              <RadioButton value="left" style="flex: 1;">图片</RadioButton>
              <RadioButton value="right" style="flex: 1;">视频</RadioButton>
            </RadioGroup>

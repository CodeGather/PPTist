<template>
  <div class="toolbar">
    ...
    <div class="content">
      ...
      <!-- <button v-for="(tab, index) in currentTabs" :key="index" @click="setToolbarState(tab.value)">{{ tab }}</button> -->
      <component :is="currentPanelComponent" :key="toolbarState.value" ></component>
    </div>
  </div>
</template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const tabs = ['Posts', 'Posts', 'Archive', 'Posts']
    let currentTabIndex = 0
    const currentTabComponent = () => {
      return 'tab-' + tabs[currentTabIndex].toLowerCase()
    }

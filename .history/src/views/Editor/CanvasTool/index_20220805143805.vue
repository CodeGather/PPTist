...
export default defineComponent({
  ...
  setup() {
    ...
    const insertImageElement = (files: File[]) => {
      ...
      uploadAttach(imageFile)
      // const uploadData = uploadAttach(imageFile)
    ...
    const getPolicy = () => {
      return new Promise<Policy>((resolve, reject) => {
        axios.get(`http://localhost:8090/jokui-dali-fast/sys/oss/batchPolicy`, {
          headers: {
    ...
    /* 上传附件 */
    const uploadAttach = async (file: File) => {
      const policyData: Policy = await getPolicy()
      const fd = new FormData()
      const fileName = policyData.dir + file.name.slice(file.name.indexOf('.'))
      fd.append('OSSAccessKeyId', policyData.accessKeyId)
      fd.append('policy', policyData.policy)
      fd.append('signature', policyData.signature)
      // 文件名字
      fd.append('key', fileName)
      fd.append('dir', policyData.dir)
      fd.append('success_action_status', '200')
      // 文件必须声明在最后，否则异常
      fd.append('file', file)
      return new Promise((resolve, reject) => {
        axios.post(policyData.host, fd, {
          headers: {
          ...
          if (res.status === 200) {
            createImageElement(policyData.host + '/' + fileName)
          }
          console.log(res)
          resolve(res)
        }).catch(err => {

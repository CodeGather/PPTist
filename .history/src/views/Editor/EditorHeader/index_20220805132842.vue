<template>
  <div class="editor-header">
    <div class="left">
      <Dropdown :trigger="['click']">
        <div class="menu-item"><IconFolderClose /> <span class="text">文件</span></div>
        <template #overlay>
          <Menu>
            <MenuItem @click="exportJSON()">导出 JSON</MenuItem>
            <MenuItem @click="exportPPTX()">导出 PPTX</MenuItem>
            <MenuItem @click="formatExportJSON()">处理 JSON</MenuItem>
            <MenuItem v-if="loadingStatus == PageDataStatus.SUCCESS" @click="savePPtxToJson()">保存 JSON</MenuItem>
          </Menu>
        </template>
      </Dropdown>
      <Dropdown :trigger="['click']">
        <div class="menu-item"><IconEdit /> <span class="text">编辑</span></div>
        <template #overlay>
          <Menu>
            <MenuItem @click="undo()">撤销</MenuItem>
            <MenuItem @click="redo()">重做</MenuItem>
            <MenuItem @click="createSlide()">添加页面</MenuItem>
            <MenuItem @click="deleteSlide()">删除页面</MenuItem>
            <MenuItem @click="toggleGridLines()">{{ showGridLines ? '关闭网格线' : '打开网格线' }}</MenuItem>
            <MenuItem @click="resetSlides()">重置幻灯片</MenuItem>
          </Menu>
        </template>
      </Dropdown>
      <Dropdown :trigger="['click']">
        <div class="menu-item"><IconPpt /> <span class="text">演示</span></div>
        <template #overlay>
          <Menu>
            <MenuItem @click="enterScreeningFromStart()">从头开始</MenuItem>
            <MenuItem @click="enterScreening()">从当前页开始</MenuItem>
          </Menu>
        </template>
      </Dropdown>
      <Dropdown :trigger="['click']">
        <div class="menu-item"><IconHelpcenter /> <span class="text">帮助</span></div>
        <template #overlay>
          <Menu>
            <MenuItem @click="goIssues()">意见反馈</MenuItem>
            <MenuItem @click="hotkeyDrawerVisible = true">快捷键</MenuItem>
          </Menu>
        </template>
      </Dropdown>
    </div>

    <div class="right">
      <!-- <Tooltip :mouseLeaveDelay="0" title="幻灯片放映">
        <div class="menu-item" @click="enterScreening()">
          <IconPpt size="18" fill="#666" style="margin-top: 2px;" />
        </div>
      </Tooltip>
      <a href="https://github.com/pipipi-pikachu/PPTist" target="_blank">
        <div class="menu-item"><IconGithub size="18" fill="#666" /></div>
      </a> -->
      <ButtonGroup class="row" v-if="orderPageData && loadingStatus == PageDataStatus.SUCCESS">
        <Button :type="toolbarState == tab.value ? 'dashed' : 'text'" v-for="tab in currentTabs" :key="tab.value" @click="setToolbarState(tab.value)">{{tab.label}}</Button>
      </ButtonGroup>
      <Tooltip 
        :mouseLeaveDelay="0" 
        title="无法识别订单，登录Token, 请前往重新登录思预云系统后进行跳转至该页面，您也可在当前页面编辑后获取JSON文件上传于系统即可" 
        placement="bottomRight"
        v-if="loadingStatus == PageDataStatus.ERROR">
        <div class="menu-item">
          <span style="color: red;">数据获取失败, 请检查是否登录！</span><IconHarm size="22" fill="#ff0000" :style="{'marginTop': '2px'}" />
        </div>
      </Tooltip>
      <IconLoading size="22" v-if="loadingStatus == PageDataStatus.LOADING" class="animation-box animate__animated animate__infinite anticon-spin"/>
    </div>

    <Drawer
      width="320"
      placement="right"
      :visible="hotkeyDrawerVisible"
      @close="hotkeyDrawerVisible = false"
    >
      <HotkeyDoc />
    </Drawer>

    <FullscreenSpin :loading="exporting" tip="正在导出..." />
  </div>
</template>

<script lang="ts">
import { computed, defineComponent, ref, getCurrentInstance } from 'vue'
import { MutationTypes, useStore } from '@/store'
import useScreening from '@/hooks/useScreening'
import useSlideHandler from '@/hooks/useSlideHandler'
import useHistorySnapshot from '@/hooks/useHistorySnapshot'
import useExport from '@/hooks/useExport'
import { ToolbarState, ToolbarStates } from '@/types/toolbar'

import HotkeyDoc from './HotkeyDoc.vue'
import { PrepareOrderInitData, PptxDataConfig } from '@/types/pageData'
import { PageDataStatus } from '@/types/pageDataStatus'

import axios from 'axios'
import { getLocationHost, getSearchPathValue } from '@/utils/common'

export default defineComponent({
  name: 'editor-header',
  components: {
    HotkeyDoc,
  },
  setup() {
    const store = useStore()
    const queryData = ref({})
    const apps:any = getCurrentInstance()

    const { enterScreening, enterScreeningFromStart } = useScreening()
    const { createSlide, deleteSlide, resetSlides } = useSlideHandler()
    const { redo, undo } = useHistorySnapshot()
    const { exporting, exportJSON, exportPPTX } = useExport()

    const showGridLines = computed(() => store.state.showGridLines)
    const toggleGridLines = () => {
      store.commit(MutationTypes.SET_GRID_LINES_STATE, !showGridLines.value)
    }

    const hotkeyDrawerVisible = ref(false)

    const goIssues = () => {
      window.open('https://github.com/pipipi-pikachu/PPTist/issues')
    }

    const toolbarState = computed(() => store.state.toolbarState)
    const currentTabs = [
      { label: '项目信息', value: ToolbarStates.PROJECT },
      { label: '服务属性', value: ToolbarStates.ORDRER_TYPE },
      { label: '下单属性', value: ToolbarStates.ORDRER_ATTRIBUTE },
      { label: '服务库', value: ToolbarStates.ORDRER_DATATYPE }
    ]
    const setToolbarState = (value: ToolbarState) => {
      store.commit(MutationTypes.SET_TOOLBAR_STATE, value)
    }

    const orderPageData = computed<PrepareOrderInitData>(() => useStore().getters.getPageData)
    const loadingStatus = computed(() => store.state.pageDataStatus)

    const slides = computed(() => store.state.slides)
    const formatExportJSON = () => {
      // console.log(JSON.stringify(slides.value))
    }

    const fileDataConfig = computed<PptxDataConfig>(() => store.getters.getReportData)
    const savePPtxToJson = () => {
      queryData.value = fileDataConfig.value
      if (!queryData.value || !queryData.value['fileName']) {
        return apps.appContext.config.globalProperties.$message.error('请完善文件名称或规则！')
      }
      const reportId = getSearchPathValue('reportId')
      let url = `${getLocationHost('/sys/report/save')}`
      if (reportId) {
        url = `${getLocationHost('/sys/report/saveData')}?reportId=${getSearchPathValue('reportId')}`
        queryData.value['id'] = reportId
      }

      const cloneData = JSON.parse(JSON.stringify(slides.value))
      queryData.value['content'] = cloneData.map((item:any) => {
        for (let i = 0; i < item['elements'].length; i++) {
          item['elements'][i]['content'] = (item['elements'][i]['content'] || '').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;')
          if (item['elements'][i]['text']) {
            item['elements'][i]['text']['content'] = item['elements'][i]['text']['content'].replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;')
          }
          item['elements'][i]['sort'] = i
        }
        return item
      })
      store.commit(MutationTypes.PAGE_SPINNING, true)
      // queryData.value['content'] = slides.value
      // JSON.stringify(slides.value).replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;')
      axios({
        url: url,
        method: 'POST',
        headers: {
          'token': getSearchPathValue('token')
        },
        data: queryData.value,
      }).then(({ data }) => {
        if (data['code'] === 0) {
          apps.appContext.config.globalProperties.$message.success(data['msg'])
          const url = `?${location.search.replace('?', '').replace(new RegExp('(^|&)reportId=([^&]*)(&|$)'), '')}&reportId=${data.reportData['id']}`
          history.pushState({url: url}, document.title, url)
          orderPageData.value['reportData'] = data.reportData
          store.commit(MutationTypes.PAGE_DATA, orderPageData.value)
        }
        else {
          apps.appContext.config.globalProperties.$message.error(data['msg'])
        }
        store.commit(MutationTypes.PAGE_SPINNING, false)
      })
    }
    return {
      redo,
      undo,
      showGridLines,
      hotkeyDrawerVisible,
      exporting,
      enterScreening,
      enterScreeningFromStart,
      createSlide,
      deleteSlide,
      toggleGridLines,
      resetSlides,
      exportJSON,
      exportPPTX,
      goIssues,
      setToolbarState,
      toolbarState,
      currentTabs,
      orderPageData,
      PageDataStatus,
      loadingStatus,
      savePPtxToJson,
      formatExportJSON,
    }
  },
})
</script>

<style lang="scss" scoped>
.editor-header {
  background-color: #fff;
  user-select: none;
  border-bottom: 1px solid $borderColor;
  display: flex;
  justify-content: space-between;
  padding: 0 10px;
}
.left, .right {
  display: flex;
  justify-content: center;
  align-items: center;
}
.menu-item {
  height: 100%;
  display: flex;
  justify-content: center;
  align-items: center;
  font-size: 14px;
  padding: 0 10px;
  transition: background-color $transitionDelay;
  cursor: pointer;

  .text {
    margin-left: 4px;
  }
}

.left .menu-item:hover {
  background-color: $lightGray;
}
</style>
    ...
    >
      ...
      >
        ...
        <div class="shape-text" :class="text.align">
          <div class="ProseMirror-static" v-html="formatTextContent(text.content)"></div>
        </div>
      </div>
    </div>
  </div>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const formatTextContent = computed(() => {
    ...
    const text = computed<ShapeText>(() => {
      ...
      const textData = JSON.parse(JSON.stringify(props.elementInfo.text))
      textData.content = textData.content.replaceAll('&lt;', '<').replaceAll('&gt;', '>').replace('&quot;', '"')
      return textData
    })

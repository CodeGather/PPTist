  ...
  >
    ...
    >
      ...
      >
        ...
        >
          <!-- 物料记录中的表格不可编辑时关闭双击编辑提示 -->
          <div 
            class="mask-tip" 
            v-if="currentSlide.keyName != 'material'"
            :style="{ transform: `scale(${ 1 / canvasScale })` }">双击编辑</div>
        </div>
      </div>
    </div>
  </div>
...
import { PPTTableElement, TableCell, Slide } from '@/types/slides'
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const startEdit = () => {
      // 物料记录中的表格不可编辑
      if (!props.elementInfo.lock && currentSlide.value.keyName !== 'material') editable.value = true
    }

    ...
    for (const slide of slides.value) {
      ...
      for (const el of slide.elements) {
        if (el.type === 'text') {
          ...
          const chartData = []
          for (let i = 0; i < el.data[0].series.length; i++) {
            const item = el.data[0].series[i]
            chartData.push({
              name: `系列${i + 1}`,
          ...
          if (el.chartType === 'bar') {
            ...
            if (el.options?.showArea) type = pptx.ChartType.area
              ...
              chartData.unshift({ name: 'X-Axis', values: Array(el.data[0].series[0].length).fill(0).map((v, i) => i) })
              options.lineSize = 0
            }

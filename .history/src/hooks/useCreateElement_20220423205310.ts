...
export default () => {
   ...
   * @param src 图片地址
   */
  const createImageElement = (src: string, keyName?: string, position?: CommonElementPosition) => {
    getImageSize(src).then(({ width, height }) => {
      ...
      createElement({
        ...
        width,
        height,
        left: (position?.left || 0) || (VIEWPORT_SIZE - width) / 2,
   ...
   * @param content 文本内容
   */
  const createTextElement = (position: CommonElementPosition, content = '请输入内容', keyName?: string) => {
    ...
    createElement({
      ...
      top: top || (VIEWPORT_SIZE * viewportRatio.value - height + 150) / 2,
      width, 
      height,
      content,
      rotate: 0,
   ...
   * @param src 视频地址
   */
  const createVideoElement = (src: string, keyName?: string) => {
    createElement({
      type: 'video',
      keyName: keyName,
      id: createRandomCode(),

...
export const mutations: MutationTree<State> = {
  ...
  // pageInitData
  [MutationTypes.PAGE_DATA](state, pageData) {
    ...
    if (pageData.reportData) {
      ...
      if (pageData.reportData.content && pageData.reportData.content instanceof Array) {
        // 处理shape数据的html代码
        pageData.reportData.content = pageData.reportData.content.map((item:Slide) => {
          item.elements = item.elements.map((element:PPTElement) => {
  ...
  // pageInitDataStatus
  [MutationTypes.PAGE_LOADING](state, pageDataStatus) {
    state.pageDataStatus = pageDataStatus
  },
  [MutationTypes.UPDATE_FILEDATA](state, data) {
    state.fileDataConfig = { ...state.fileDataConfig, ...data }
  },
  [MutationTypes.PAGE_SPINNING](state, data) {
    state.spinning = data
  }
}